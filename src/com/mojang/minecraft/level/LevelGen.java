...
public class LevelGen {
   ...
   public byte[] generateMap() {
      ...
      for(y = 0; y < x; ++y) {
         float _x = this.random.nextFloat() * (float)w;
         float _y = this.random.nextFloat() * (float)d;
         ...
         for(int l = 0; l < length; ++l) {
            _x = (float)((double)_x + Math.sin((double)dir1) * Math.cos((double)dir2));
            z = (float)((double)z + Math.cos((double)dir1) * Math.cos((double)dir2));
            _y = (float)((double)_y + Math.sin((double)dir2));
            dir1 += dira1 * 0.2F;
            dira1 *= 0.9F;
            ...
            for(int xx = (int)(_x - size); xx <= (int)(_x + size); ++xx) {
               for(int yy = (int)(_y - size); yy <= (int)(_y + size); ++yy) {
                  for(int zz = (int)(z - size); zz <= (int)(z + size); ++zz) {
                     float xd = (float)xx - _x;
                     float yd = (float)yy - _y;
                     float zd = (float)zz - z;
